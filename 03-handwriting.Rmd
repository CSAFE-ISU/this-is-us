...
Now to run multiple simulations using a loop. We randomly select n writers. Then randomly choose an unknow/qd. To select s, we rank the rest of the writers by their Euclidean distance of the proportion of graphs. The one that is most similar to u gets assigned to be s and the rest are a. Then run through bayes factor code. The idea is that we are randomly selecting writers and checking bayes factors for writers that are most similar to our unknown (kinda like a close non match). Do this for all writers in our selected n sample. We want to be checking more for unknowns where the specific source is not the same than when it is the same (qd=36, s=71 instead of qd=36 s=36).  We also do this with euclidean distances that are far. The idea is that we will have two cases where Hd is true, one that is an easy difference and one pthat is difficult. We also run a loop for when Hp is true and u=s. 
