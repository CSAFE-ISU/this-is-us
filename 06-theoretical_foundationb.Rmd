...
Practitioner oftens uses $(𝑪_{𝒄𝒔_𝑷},𝑪_{𝒄𝒔_D})$ directly while developing an SLR system.
...
We can organize our predictions into a matrix were each row represents an observation and there are $M$ columns each asociated with a particular BSLR.
...
If an aditional set is available, aditional step can be done.
...
So we consider a combined wheigth for $RME$ and $DP$.
