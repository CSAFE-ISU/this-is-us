...
There is one holdout document for each writer. The $\boldsymbol{\bar{p}}$ vector as given above is shown graphically for each holdout document (the rows).
...
\alpha_{k}, \: \beta_{k} \: &\stackrel{iid}{\sim}\: Gamma(2, \: 0.25),  \\
...
Investigating post-processing methods to handle identifiability issues with this model. The MCMC routine is stuck in one of two modes (which saves us, here). Making the usual inferences about $\rho, \alpha,$ and $\beta$ is not appropriate if the label switching problem is not addressed.
