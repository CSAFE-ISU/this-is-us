...
**cartridge cases **
  ...
  - Cadre TopMatch Scanner: when taking scans of breech face images, we are following NIBIN protocol as losely as possible.
...
After all the regions have been correctly annotated and saved, the x3p file should contain a land engraved area scan with additional data known as a “mask.”
    ...
    ```{r, fig.cap="Title coming soon.", fig.keep="hold", fig.align='center', out.width="40%"}
...
```{r, fig.cap="Title coming soon.", fig.keep="hold", fig.align='center', out.width="40%"}
knitr::include_graphics(c("images/andy_temp/Missing_Values_Interpolation_1.png", "images/andy_temp/Missing_Values_Interpolation_3.png"))
```
...
```{r, fig.cap="Title coming soon.", fig.keep="hold", fig.align='center', out.width="10%"}
...
```{r, fig.cap="Title coming soon.", fig.keep="hold", fig.align='center', out.width="50%"}
...
Since I only used 442 barrels out of 626 total barrels in the LAPD bullet data set, we started a review process to check those "problematic" LEA scans. Some of the work done and findings are listed below:
...
4. For the remaining unused data, there are some issues like: units mistake (solved now), bullets not 4 (reached an agreement to solve), lands not 6 (reached an agreement to solve). For some used data, there are also issue like: inconsistent names (lands and barrels),  mislabeling (same land shown up twice). Those are all identified to my best.
...
Besides, I also did some writing which is the main part to do for time from now.
