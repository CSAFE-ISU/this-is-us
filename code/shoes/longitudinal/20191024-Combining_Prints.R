...
df2 <- df %>%
  mutate(
    file = str_replace(file, "image", "aligned_composite"),
    img = purrr::map(file, readImage)
  )

df2_wide <- df2 %>% select(-file, -base) %>%
  mutate(useful_frame = purrr::map(img, ~({
    x <- .[,,3]
    colorMode(x) <- 0
    img_rotate(x, 90) %>%
...
# Full color/mask/binary layers
df3_wide <- df2 %>% select(-file, -base) %>%
  mutate(useful_frame = purrr::map(img, ~({
    img_rotate(., 90) %>%
      img_autocrop()
  }))) %>%
...
par(mfrow = c(3, 9))
purrr::walk(c(df_wide$joint, df2_wide$joint, df3_wide$joint), plot)

## Conclusion: the third method is the best at preserving contextual information.

## Now to combine the two images to get a more complete image:
## pmin(im1, im2) will keep all the noise from both images, but in the cropped
##                images, there isn't that much noise anyways...
## pmax(im1, im2) would keep only pixels which are in both images
## mean(im1, im2) would result in lighter variations where there are transitions from one to another image.
## Of these, pmin() seems like the best.
...
df3_wide <- df3_wide %>%
  mutate(joint_min = purrr::map(joint, ~pmin(.[,,1], .[,,2], .[,,3]) %>% toGray()),
         joint_max = purrr::map(joint, ~pmax(.[,,1], .[,,2], .[,,3]) %>% toGray()),
         joint_mean = purrr::map(joint, ~((.[,,1] + .[,,2] + .[,,3])/3) %>% toGray()))

par(mfrow = c(1, 9))
purrr::walk(df3_wide$joint_min, plot)

par(mfrow = c(1, 9))
purrr::walk(df3_wide$joint_max, plot)

par(mfrow = c(1, 9))
purrr::walk(df3_wide$joint_mean, plot)
