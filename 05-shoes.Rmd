...
<img src="images/shoes/3dshoes/SidePACwithallpoints.png" width="100%"/>
...
This is working wiht the shoes that I have personally scanned, however still in the process of working on fixing them for the shoes of the longitudinal study. One thing that seems to be working is when we compare two shoes of the same size that were both turn tables we seem to get a better match. 
...
**Comments by reviewers**
  ...
  1. Discuss ways in which the MC-COMP could be **tuned** to fit a more **generalizable** prediction?
      + image pattern matching including other forensic discipline. 
    
  2. Possible to extend to **3D images**?
  3. Possible to include manual matching from **a forensic examiner**? 
  4. Compare MC-COMP with **deep learning approaches**.
      + Kong et al. (2019); aligning two images, find features by CNN, predict the source by ResNet-50 model. 
    
  5. What **other classifiers** we tried, besides random forest?
    
  6. Define a **variability** in the similarity score.
      + Train Bayesian Additive regrssion trees (BART)
    
  7. Compare your predictive accuracies on the **number of circles (more or less than 3)**.
